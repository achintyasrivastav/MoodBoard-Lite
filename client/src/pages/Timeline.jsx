...
export default function Timeline() {
  ...
  useEffect(() => {
    // Uses the correct backend route: GET /api/mood/history
    api
  ...
  // group by date (YYYY-MM-DD) returned by backend
  const groups = useMemo(() => {
    const map = new Map();
    for (const m of items) {
      const key = m.date; // backend stores normalized day string
      if (!map.has(key)) map.set(key, []);
      map.get(key).push(m);
    }
    // keep API’s reverse-chronological order
  ...
  return (
    <div className="mx-auto max-w-4xl p-4">
      <h2 className="text-xl text-slate-200 font-semibold mb-6">Timeline</h2>
                    ...
                    >
                      ...
                      <article className="rounded-2xl border border-slate-800 bg-slate-900 p-4 shadow">
                        <header className="mb-2 flex items-center justify-between">
                          <div className="text-2xl">{Array.isArray(m.emojis) ? m.emojis.join(" ") : ""}</div>
                          <time className="text-xs text-slate-400">{fmtTime(m.createdAt || m.updatedAt)}</time>
                        </header>

                        {m.color && (
                          <div className="h-1.5 w-full rounded-full mb-3" style={{ backgroundColor: m.color }} />
                        )}
                        ...
                        {m.note && <p className="text-sm text-slate-300 leading-relaxed">{m.note}</p>}
                      </article>
